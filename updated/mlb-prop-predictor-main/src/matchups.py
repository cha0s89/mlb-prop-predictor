...
def get_bvp_matchup(batter_id: int, pitcher_id: int, years_back: int = 3) -> dict:
    ...
    try:
        ...
        # Count plate appearances (unique game + at_bat combos, since at_bat_number resets per game)
        if "game_pk" in matchup.columns and "at_bat_number" in matchup.columns:
            pa = matchup.drop_duplicates(subset=["game_pk", "at_bat_number"]).shape[0]
        elif "at_bat_number" in matchup.columns:
            pa = matchup["at_bat_number"].nunique()
        else:
            pa = 0
